...
{	
	...
	III_CHAR ch="";         //從header擷取出來的資料
	III_CHAR RTID[256];     //RouteTable 識別
	III_INT STARTTIME = 0;
	III_INT ENDTIME = 0;
	III_INT CACHEINITSTARTTIME = 0;
	III_INT CACHEINITENDTIME = 0;
	III_CHAR buf2[256]="";  //For Performance Test

	STARTTIME = GetTickCount();
	a->area = (Area*)III_malloc(sizeof(Area));
	...
	//Init close list
	a->close.visitEdge = (III_CHAR**)III_malloc(sizeof(void*));
	a->close.visitEdge[0] = NULL;
	a->close.list = (Edge2**)III_malloc(sizeof(void*)*LENGTHM);
...
III_INT AstarSetStart(Astar *a, III_INT PointX, III_INT PointY, III_INT Fx, III_INT Fy, III_INT Tx, III_INT Ty)
{
	...
	ENDTIME = GetTickCount() - STARTTIME;
	//sprintf(buf2, "API_SaveEdgeInfo_SPEND_TIME = %d \n", ENDTIME);   
	III_fwrite(buf2, 1, strlen(buf2), performaceTestFile);
	...
	ENDTIME = GetTickCount() - STARTTIME;
	//sprintf(buf2, "API_CacheRead_SPEND_TIME = %d \n", ENDTIME);   
	III_fwrite(buf2, 1, strlen(buf2), performaceTestFile);
...
III_INT AstarRouting(Astar *a,  III_RESULTPDRT* resultPDRT)
{
	...
	ENDTIME = GetTickCount() - STARTTIME;
	//sprintf(buf2, "API_AstarSetWeight_SPEND_TIME = %d \n", ENDTIME);   
	III_fwrite(buf2, 1, strlen(buf2), performaceTestFile);
	...
	ENDTIME = GetTickCount() - STARTTIME;
	//sprintf(buf2, "API_RunAstar2_SPEND_TIME = %d \n", ENDTIME);   
	III_fwrite(buf2, 1, strlen(buf2), performaceTestFile);
