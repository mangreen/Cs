...
III_INT AstarSetWeight(Astar *a)
{
	...
	if(MODE_PD_SHORTEST == a->mode)
	{
		...
		RW[ACCESSABLE] = 36; 
		RW[STAIR]= 36; 
		RW[SHELTER|ACCESSABLE] = 36; 
		RW[SHELTER|STAIR] = 36;
		RW[ACCESSABLE|STAIR] = 36; 
		RW[SHELTER|ACCESSABLE|STAIR] = 36;
		return ERROR_SUCCESS;
	}
...
III_INT RunAstar2(Astar *a, III_RESULTPDRT*  resultPDRT)
{
	...
	III_CHAR buf2[256];								//FOR PERFORMANCE TEST
	...
	//sprintf(buf2, "API_AstarClean_SPEND_TIME = %d \n", ENDTIME);   
	III_fwrite(buf2, 1, strlen(buf2), performaceTestFile);
	...
	tempMax = tempMax*2+1; 
	a->open.heap = (Edge*)III_malloc(sizeof(Edge)*(a->open.limit = MAXOPENLIST/2));
	a->close.visitEdge[0] = (III_CHAR*)III_malloc(tempMax/8+1);
	...
	for(i=0;i<2;i++)
	{
		...
		edge.Pg = sqrt(abs(a->start.mx - edge.x) * abs(a->start.mx - edge.x) + abs(a->start.my - edge.y) * abs(a->start.my - edge.y)) / 11;
	...
	ENDTIME = GetTickCount() - STARTTIME;
	//sprintf(buf2, "STARTEDGE_Push_SPEND_TIME = %d \n", ENDTIME);   
	III_fwrite(buf2, 1, strlen(buf2), performaceTestFile);
	...
	while(0 == a->stopFlag)
	{
		...
		if((edge.eid>>1) == a->end.eid)
		{
			...
			if(0 == edge.eid %2)
			{
				edge.Pg =  sqrt(abs(a->end.mx - a->end.fx) * abs(a->end.mx - a->end.fx) + abs(a->end.my - a->end.fy) * abs(a->end.my - a->end.fy)) /11;
			}
			else
			{
				edge.Pg =  sqrt(abs(a->end.mx - a->end.tx) * abs(a->end.mx - a->end.tx) + abs(a->end.my - a->end.ty) * abs(a->end.my - a->end.ty)) /11;
			}
		...
		for(ptr=buf, i=0; i<num; i++, ptr+=ADJSIZE)
		{
			...
			{
				RW[adj.type] = 10;
				//printf("newG = %d, adj.dist = %d, RW[adj.type] = %d, adj.type = %d \n", newG, adj.dist, RW[adj.type] , adj.type);
			}
			else
			{
				RW[adj.type] = 36;
	...
	ENDTIME = GetTickCount() - STARTTIME;
	//sprintf(buf2, "AstarAlgorithm_Routing_SPEND_TIME = %d \n", ENDTIME);   
	III_fwrite(buf2, 1, strlen(buf2), performaceTestFile);

	III_free(a->open.heap);
	a->open.heap = NULL;
	...
	ENDTIME = GetTickCount() - STARTTIME;
	//sprintf(buf2, "API_PGetPath_SPEND_TIME = %d \n", ENDTIME);   
	III_fwrite(buf2, 1, strlen(buf2), performaceTestFile);
	...
	//sprintf(buf2, "API_FreeCloselist_SPEND_TIME = %d \n", ENDTIME);   
...
III_INT SetVisit(CloseList *close, Edge *e)
{
	...
	if(0 == remainder)
	{
		close->list[quotient] = (Edge2*)III_malloc(sizeof(Edge2)*LENGTHN);
	}
...
{ //from h1 to h2
	...
	temp = (Edge*)III_malloc(sizeof(Edge)*open->count);
...
III_INT PGetPath(CloseList *close, III_RESULTPDRT* resultPDRT)
{
	...
	resultPDRT->resultEdge = III_malloc(sizeof(III_PDRTEDGE) * edgeTotalCount);
