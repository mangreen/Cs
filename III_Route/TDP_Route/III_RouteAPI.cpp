...
III_INT III_RouteAPI::GetNearEdge(III_INT X, III_INT Y, III_RTPOINT *SEPoint)
{
	III_INT dist = 0;
	III_INT range = 0;
	III_INT mx;
...
III_INT III_RouteAPI::ComputeDist(III_INT x, III_INT y, III_INT *X, III_INT *Y, III_INT n, III_INT *mx, III_INT *my)
{
...
III_INT III_RouteAPI::readpoints(III_UCHAR *buf, III_INT Ptr, III_USHORT poNum, III_INT * ptsX, III_INT * ptsY) 
{
	III_SHORT dx, dy;
...
III_INT  III_RouteAPI::GetStData(struct SObjData *objelem , III_FILE *pdtfd, struct MdbStreet ** stRecord)
{
	III_UCHAR *buf;
	III_INT Ptr = 0;
	III_UINT stAttr[2];
	...
	buf = (III_UCHAR *)malloc ( objelem->size );
	...
	(*stRecord)->m_ptsX = (III_INT *)malloc ( sizeof ( III_INT ) * poNum );
	(*stRecord)->m_ptsY = (III_INT *)malloc ( sizeof ( III_INT ) * poNum );
...
III_VOID III_RouteAPI::QueryStreetByID(III_UINT sid, III_FILE *idxfd, III_FILE *pdtfd, struct MdbStreet ** stobj)
{
	//將傳入sid建成linking list結構
	III_INT tarr[2];
	struct SObjData *objelem = (struct SObjData *)malloc ( sizeof ( struct SObjData ) );
	objelem->objID = sid;
...
III_VOID III_RouteAPI::initialMdbStreet(struct MdbStreet *st) 
{
	st->m_sid = 0;
	st->m_countryID = 0;
	st->m_ptNum = 0;
...
III_VOID III_RouteAPI::freeSt(struct MdbStreet ** head) 
{
...
III_VOID III_RouteAPI::quicksort(III_INT *arr, III_INT left, III_INT right) 
{
...
III_INT III_RouteAPI::Overlap(SRect *Rect1, SRect *Rect2) 
{
	return ( !( Rect1->x_min > Rect2->x_max || Rect1->y_min > Rect2->y_max || Rect2->x_min > Rect1->x_max
			|| Rect2->y_min > Rect1->y_max ) );
}

III_INT * III_RouteAPI::GetObjID2(SRect *rect, III_INT *count, III_FILE *rtxfd)
{
	
	III_BOOL flag = 1;
	SR_Record *sr = (SR_Record *)malloc ( sizeof ( SR_Record ) * 6 );
	III_INT n[6];
	III_INT lvl = 0;
	III_INT *arr2;
	III_INT c = 0;
	III_INT **arr = (III_INT**)malloc(sizeof(void*)*256); 
	III_INT t1;
	III_INT t2;
	III_SHORT maxL;
	...
	while ( flag )
	{
		if ( sr[lvl].Level == 0 )
		{
			if ( Overlap ( rect, &sr[lvl].Entry[n[lvl]].Rect ) )
			{
				...
				if ( t2 == 0 )
				{
					arr[t1] = (III_INT *)malloc ( sizeof ( III_INT ) * 512 );
				}
	...
	*count = c;
	t1 = ( c - 1 ) >> 9;
	arr2 = (III_INT *)malloc ( sizeof ( III_UINT ) * ( t1 + 1 ) * 512 );
...
//Road Lock
III_VOID III_RouteAPI::readStreetData(struct ListStreet *stList, SRect rect)
{
	...
	strcat(pRtTablePath, "III_RoadLock\\routeTable-rd.rtx");           
	fileSet.rtxfd = III_fopen ( pRtTablePath, "rb" );

	strcpy(pRtTablePath, g_szMapFolder);
	strcat(pRtTablePath, "III_RoadLock\\routeTable-rd.idx");           
	fileSet.idxfd = III_fopen ( pRtTablePath, "rb" );

	strcpy(pRtTablePath, g_szMapFolder);
	strcat(pRtTablePath, "III_RoadLock\\routeTable-rd.pdt");           
	fileSet.pdtfd = III_fopen ( pRtTablePath, "rb" );
	...
	while ( c != count && cur)
	{
			...
			{ //insert
				temp = (struct MdbStreet *)malloc ( sizeof ( struct MdbStreet ) );
...
*/
III_INT III_RouteAPI::III_SetRoutingParam(III_RTPARAM *paramRounting)
{
	...
	if ( TYPE_ROUTE_PD == typetoroute)
	{
		 ...
		 *			 [out] see III_Route_Manager.h error definition
		 */
		result_process = III_PD_SetRoutingParam(paramRounting);
	}
	else if ( TYPE_ROUTE_INDOOR == typetoroute)
	{
		result_process = III_Indoor_SetRoutingParam(paramRounting);
	}
	else // if ( TYPE_ROUTE_MT == typetoroute)
	{
//		III_MT_SetRoutingParam(paramRounting);
	}
...
*			[out] see III_Route_Manager.h error definition
*/
III_INT III_RouteAPI::III_Routing(III_RTPOINT pntDeparture, III_RTPOINT pntArrival)
{
        III_INT result_process = ERROR_SUCCESS;
	if ( TYPE_ROUTE_PD == typetoroute)
	{
		 ...
		 *
		*/
		result_process = III_PD_Routing(pntDeparture, pntArrival);
	}
	else if ( TYPE_ROUTE_INDOOR == typetoroute)
	{
		result_process = III_Indoor_Routing(pntDeparture, pntArrival);
	}
	else // if ( TYPE_ROUTE_MT == typetoroute)
	{
//		III_MT_Routing(pntDeparture, pntArrival);
	}
	return result_process;
}
 ...
 *		 [out] see III_Route_Manager.h error definition
*/
III_INT III_RouteAPI::III_GetPdRTResult(III_RESULTPDRT *resultPDRT)
{
        III_INT result_process = ERROR_SUCCESS;

	if ( TYPE_ROUTE_PD == typetoroute)
	{
		result_process = III_PD_GetPdRTResult(resultPDRT);
	}
	return result_process;
}
 ...
 *		 [out] see III_Route_Manager.h error definition
*/
III_INT III_RouteAPI::III_GetIdrRTResult(III_RESULTINDOOR *resultIdrRT)
{
       III_INT result_process = ERROR_SUCCESS;

	if ( TYPE_ROUTE_INDOOR == typetoroute)
	{
		result_process = III_Indoor_GetIdrRTResult(resultIdrRT);
	}
...
/*
III_INT III_RouteAPI::III_GetMTRTResult(III_RESULTMTRT *resultMTRT)
{
	III_MT_GetMTRTResult(resultMTRT);
