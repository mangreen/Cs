...
int startLockRoad(III_RTPOINT *rtPoint, char *lockName){
	...
	idrLR = (IndoorLockRoad*)malloc(sizeof(IndoorLockRoad));   // 弄個新空間;
...
int initialLockRoad(IndoorLockRoad *idrLR, char *lockName){
	...
	idrLR->lockfile = (FILE *)III_fopen(lockName, "rb");
	if(!idrLR->lockfile) {
...
int getFloorEdgeNum(IndoorLockRoad *idrLR, int floor){
	...
	idrLR->leTable.lockEdge = (LockEdge*)malloc(sizeof(LockEdge)*(idrLR->leTable.floorEdgeNum));   // 弄個新空間;
